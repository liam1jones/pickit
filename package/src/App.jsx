...
/** Stable QR payload for a ticket. New tickets store their own `qrPayload` at
 *  creation; older/seed tickets fall back to the ticket id so they still scan. */
const ticketQrValue=(t)=>t?.qrPayload||`pickit://ticket/${t?.id||""}`;

const parseLumaUsd=(s)=>{if(!s||typeof s!=="string")return null;const n=Number.parseFloat(s.replace(/[$,\s]/g,""));return Number.isFinite(n)?n:null;};
/** Rows for PARTS_CATALOG["Optics / Transceivers"] — sourced from `lumaOptics.ts` (deduped by part #, sorted). */
const LUMA_OPTICS_CATALOG_PARTS=(()=>{
  const byPn=new Map();
  for(const it of lumaOpticsCatalog){
    const raw=(it.partNumber||"").trim();
    if(!raw)continue;
    const key=raw.toUpperCase();
    const price=parseLumaUsd(it.estimatedCost);
    const desc=((it.description||it.model||"").trim())||raw;
    byPn.set(key,{pn:raw,desc,price:price??0});
  }
  return [...byPn.values()].sort((a,b)=>a.pn.localeCompare(b.pn,"en",{numeric:true,sensitivity:"base"}));
})();
...
const PARTS_CATALOG={
  ...
  "Optics / Transceivers": LUMA_OPTICS_CATALOG_PARTS,
...
const LUMA_PRICE_BY_PN=(()=>{const m=new Map();for(const it of lumaOpticsCatalog){const p=parseLumaUsd(it.estimatedCost);const k=(it.partNumber||"").trim().toUpperCase();if(k&&p!=null)m.set(k,p);}return m;})();
/** Unit price for BOM display: Luma catalog `estimatedCost` when part matches, else stored unitPrice. */
const bomUnitPrice=(item)=>{const k=(item.partNumber||"").trim().toUpperCase();if(k&&LUMA_PRICE_BY_PN.has(k))return LUMA_PRICE_BY_PN.get(k);return item.unitPrice||0;};
...
export default function App(){
  ...
  function submitTicket(){
    ...
    const t={id,site:form.site,lines,location:loc,qtyReq:lines.reduce((s,l)=>s+l.qtyReq,0),qtyIns:null,qtyRet:null,req:user.name,ics:form.ics,dcm:form.dcm||null,taskLink:form.taskLink||"",projectId:null,status:"Pending Approval",date:tod(),comments:[],asana:"",qrPayload,log:[{ts:now(),who:user.name,role:user.role,action:"Ticket submitted",detail},{ts:now(),who:user.name,role:user.role,action:"QR tag assigned",detail:qrPayload}]};
    setTickets(p=>[t,...p]);
